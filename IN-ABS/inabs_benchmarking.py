...
# === Load dataset ===
with open("/home/rs/20CS91R17/IN-ABS/pairs.jsonl", "r", encoding="utf-8") as f:
    lines = f.readlines()
...
def build_prompt(article):
    return f"""You are Indian legal expert. Summarize the following case court judgement into a summary of less than 500 words.
...
for model_id, model_name in models.items():
    ...
    with open(f"inabs_tok_sum_{model_id}.txt", "w", encoding="utf-8") as summary_file:
        for record in tqdm.tqdm(data):
            try:
                ...
                input_len = len(article)  # characters in the input article
                prompt = build_prompt(article)
                input_tokens = len(tokenizer.tokenize(prompt))  # token count of the prompt

                tracker = EmissionsTracker(
                    project_name=f"INABS_{model_id}",
                    output_dir="./",
                    log_level="error",
                    save_to_file=False
                )
                ...
                # Extract summary and calculate token & char length
                summary = output.split("Summary:")[-1].strip().split("\n")[0]
                output_len = len(summary)  # characters in the output
    ...
    with open(f"inabs_tok_{model_id}.csv", "w", newline='', encoding="utf-8") as f:
        writer = csv.writer(f)
...
with open("inabs_tok_results.csv", "a", newline='', encoding="utf-8") as f:
    writer = csv.writer(f)
