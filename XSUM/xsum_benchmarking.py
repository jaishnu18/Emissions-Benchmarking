...
# === Load dataset ===
with open("xsum_random200.jsonl", "r", encoding="utf-8") as f:
    lines = f.readlines()
data = [json.loads(line.strip()) for line in lines]
data = sorted(data, key=lambda x: len(x["document"]))
...
for model_id, model_name in models.items():
    ...
    with open(f"xsum_sum_{model_id}.txt", "w", encoding="utf-8") as summary_file:
        for record in tqdm(data, desc=f"Summarizing with {model_id}"):
            idx = record["id"]
            article = record["document"]
    ...
    with open(f"xsum_{model_id}.csv", "w", newline='', encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["id", "input_length", "output_length", "input_tokens", "output_tokens", "energy_kwh", "carbon_kg"])
        for r in results_sorted:
            writer.writerow([
                r["id"], r["input_length"], r["output_length"], r["input_tokens"], r["output_tokens"],
...
with open("xsum_results.csv", "a", newline='', encoding="utf-8") as f:
    writer = csv.writer(f)
